...
df_17 = pd.read_csv("SUP_CURSO_2019.CSV", delimiter='|')

st.title("Cursos disponíveis nas Universidades brasileiras (2019)")
st.text('')
st.text('')
st.image('inep.jpg')
st.markdown("""
    ...
    corresponde aos mais de **40 mil** cursos ofertados pelas Universidades Brasileiras.
""")
...
if st.sidebar.checkbox('Visualizar cabeçalho da tabela? '):
    st.header("Cabeçalho da tabela SUP_CURSO_2019.CSV:")
    st.write(df_17.head())
    st.markdown("""
        **Fonte:** INEP
        """)

st.sidebar.info(f"O Dataset de 2019 completo possui {df_17.shape[0]} linhas e {df_17.shape[1]} colunas.")
...
#criando novos DataFrames com os filtros solicitados
